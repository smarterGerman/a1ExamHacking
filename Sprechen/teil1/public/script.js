...
function toolAnswer(){
  p.innerText = allTriggerAnswersData[0][current_trigger][0];
...
recognition.addEventListener('result', e => {
    ...
    if (e.results[0].isFinal){
      ...
      current_trigger = current_trigger.replace(/\n/g,"");
      let tmpArr = allTriggerAnswersData[0][current_trigger].slice(1);
      let spoken = paragraphs[paragraphs.length - 2].innerText.toLowerCase().replace(/[.,?!;:]/g,"");
      if(tmpArr.some(el => spoken.includes(el.toLowerCase())))
      {
