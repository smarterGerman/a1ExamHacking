...
function checkRadioTmp(id) {
    Solutions[id].a.forEach((element) => {
        if($('input[name="fifth"]:checked').val().trim().toLowerCase().replace(/[.,\/#!$%\^&\*;:{}=\-_`~()] /g,"") == element.text.trim().toLowerCase().replace(/[.,\/#!$%\^&\*;:{}=\-_`~()] /g,"")) {
            uncheckRadio();
            $('input[name="fifth"]:checked').parent()[0].classList.add('radio-correct');
            $('input[name="fifth"]:checked').parent()[0].classList.remove('radio-incorrect')
        } else {
            uncheckRadio();
            $('input[name="fifth"]:checked').parent()[0].classList.remove('radio-correct');
            $('input[name="fifth"]:checked').parent()[0].classList.add('radio-incorrect')
        }
    })
}
